...
- 有向路由：按点位维度配置 `src -> dst` 的转发规则，支持一对一、一对多与多对一
...
- 聚合/仲裁规则（多对一不做求和、平均、主备优选或质量择优；同步命令按目标连接顺序串行执行）
...
- 支持关系：
  ...
  - 多对一：`(1, srcTag1) -> (2, dstTag)` + `(3, srcTag2) -> (2, dstTag)` + ...；普通点值每条更新均逐条转发，按 DataCenter 服务端处理顺序进入目标订阅队列，目标最新值为最后处理的一条更新。同步命令则按目标连接的服务端受理顺序串行调用，避免多个源命令并发进入同一目标连接。
...
- `UpsertRoutes(UpsertRoutesRequest) -> Empty`
  ...
  - 支持一对一、一对多与多对一；普通点值不做聚合或仲裁，按服务端处理顺序逐条转发；同步命令按目标连接顺序串行执行。
