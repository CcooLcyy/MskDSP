...
grpc::Status DataCenterClient::PublishUInt16(
    uint32_t connId, const std::string& tag, uint16_t value, DataCenterProto::Quality quality, int64_t tsMs) {
  if (connId == 0) {
    return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "conn_id 不能为空");
  }
  if (tag.empty()) {
  ...
  req.set_conn_id(connId);
  req.set_tag(tag);
  req.mutable_value()->set_int_value(static_cast<int64_t>(value));
  if (tsMs > 0) {
    req.set_ts_ms(tsMs);
  }
  req.set_quality(quality);
...
grpc::Status DataCenterClient::PublishString(
    uint32_t connId, const std::string& tag, const std::string& value, DataCenterProto::Quality quality, int64_t tsMs) {
  if (connId == 0) {
    return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "conn_id 不能为空");
  }
  if (tag.empty()) {
  ...
  req.set_conn_id(connId);
  req.set_tag(tag);
  req.mutable_value()->set_string_value(value);
  if (tsMs > 0) {
    req.set_ts_ms(tsMs);
  }
  req.set_quality(quality);
