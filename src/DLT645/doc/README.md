...
- 若首轮 `addslaveNode` 失败，链路会保持 `STOPPED`，模块后台每 5 秒重试一次档案添加；重试成功后会自动继续启动连接功能。`StopLink`、`UpsertLink`、`UpsertPointTable` 会终止该后台重试，并以最新配置重新判定后续行为。
...
- 若首轮 `addslaveNode` 返回失败，模块会在后台每 5 秒继续重发，直到成功、连接功能被停止，或链路配置/点表被再次更新。
