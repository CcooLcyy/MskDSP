...
- 当前实现会将已下发的 MQTT/链路/点表配置本地持久化到 `./conf/config.db`，用于模块重启后的自动恢复；配置类 RPC 成功后不会自动启动模块内的链路连接功能，恢复出的链路若满足当前最小可运行条件则仍可能自动进入运行态。
...
- `state` 表示链路生命周期：`STOPPED`、`RUNNING` 或 `PENDING_DELETE`，用于控制启停操作。
- `communication_state` 表示最近一轮实际抄读的现场通信健康状态，独立于 `state`。
- 链路启动、停止或重启后先为 `UNSPECIFIED`（等待首轮抄读）。
- 一轮抄读中只要至少一个点收到合法帧并完成 DI、数据长度及数据解析，就置为 `HEALTHY`；整轮没有任何有效点则置为 `UNHEALTHY`。
- 全 `FF` 数据按设备未接入处理，不计入有效抄读；死区过滤和 DataCenter 上报失败不影响现场通信健康统计。
...
- 若首轮 `addslaveNode` 返回失败（不含数值型 `status=2`/字符串 `"2"` 的“档案已存在”），模块会在已进入启动流程后于后台每 5 秒继续重发，直到成功、连接功能被停止，或链路配置/点表被再次更新。
- `COMM_MODE_LORA` 的正式轮询点抄若收到非零 `status`，模块不会进入档案后台重试逻辑；仅在当前轮询线程内按 `request_timeout_ms` 等待后继续抄下一个点或数据块。
- `StartLink` 首次调用可能直接返回失败，但后台仍会继续重试档案添加；`UpdateConfig/UpsertLink/UpsertPointTable` 返回成功只代表配置已生效，不代表连接功能已经进入 `RUNNING`。集成侧应通过 `GetLink/ListLinks` 的 `state` 与 `last_error` 判断最终状态。
...
## 配置持久化（当前实现）
DLT645 会将模块内已生效的配置落盘到工作目录下的 `./conf/config.db`，用于进程重启后的自动恢复。

### 文件与策略
- SQLite 配置项：`DLT645/mqtt`、`DLT645/links`、`DLT645/point_tables`
...
- 模块启动后，恢复出的链路若满足上述最小条件，会自动进入运行态；若该自动恢复过程仅因 `addslaveNode` 失败、Lora/载波瞬时异常而未完成，DLT645 会记录中文日志与 `last_error`，并在后台每 5 秒重试档案添加；若是 SQLite 配置解析失败、点表或数据块非法、链路与点表不匹配等配置问题，则保持模块服务在线，等待上位机后续修正配置。
- `StartLink` RPC 是显式启动模块内链路连接功能的唯一入口，连接已在运行时直接返回成功；当首轮 `addslaveNode` 失败时，本次 RPC 可能先返回失败，但模块仍会在后台继续重试档案添加，最终是否进入运行态应以 `GetLink/ListLinks` 的查询结果为准。
