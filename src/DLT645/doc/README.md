...
- 当前实现会将已下发的 MQTT/链路/点表配置本地持久化到 `./conf/DLT645/`，用于模块重启后的自动恢复；配置类 RPC 成功后不会自动启动模块内的链路连接功能，恢复出的链路若满足当前最小可运行条件则仍可能自动进入运行态。
...
- 模块启动后，恢复出的链路若满足上述最小条件，会自动进入运行态；若该自动恢复过程仅因 `addslaveNode` 失败、Lora/载波瞬时异常而未完成，DLT645 会记录中文日志与 `last_error`，并在后台每 5 秒重试档案添加；若是 `links.pb` / `point_tables.pb` / `mqtt.pb` 解析失败、点表或数据块非法、链路与点表不匹配等配置问题，则保持模块服务在线，等待上位机后续修正配置。
