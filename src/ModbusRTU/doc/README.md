...
- 点表管理、启停轮询/响应
- 轮询数据发布到 DataCenter（BOOL 原样、UINT16 按 scale/offset 转为 double）
...
- 点表字段：tag/function/address/type、scale/offset/deadband、default_value（从站兜底）
...
- `scale/offset`：工程量换算 `value = raw * scale + offset`（`scale=0` 视为 1）；仅对 UINT16 生效。
- `deadband`：工程量单位，仅对 UINT16 生效；`|value - last_reported| < deadband` 时不上报，<=0 表示不过滤。
- `deadband` 仅对主站轮询发布生效，从站响应不受 deadband 影响。

## 从站说明
- `mode=LINK_MODE_SLAVE` 时，模块作为从站响应请求（支持 0x01/0x03）。
- 点值来源：优先读取 DataCenter 最新值并按 `scale/offset` 反向缩放；若无值或 DataCenter 不可用则使用点表 `default_value` 兜底。
...
- 读出数据将发布到 DataCenter，点值类型分别映射为 bool/double（UINT16 经 scale/offset）。
