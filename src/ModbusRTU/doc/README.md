...
## 配置持久化（当前实现）
- 当前实现会将已下发的 MQTT/链路/点表配置本地持久化到 `./conf/config.db`，用于模块重启后的自动恢复。
- SQLite 配置项：`ModbusRTU/mqtt`、`ModbusRTU/links`、`ModbusRTU/point_tables`。
...
- 若 SQLite 配置解析失败、点表非法、链路与点表不匹配、MQTT 配置不完整或自动启动失败，ModbusRTU 仅记录中文日志并保持模块服务在线，等待上位机后续修正配置。
- `StartLink` RPC 是显式启动模块内链路连接功能的唯一入口，并保持幂等语义：链路已在运行时直接返回成功。
- `last_error`、总线实例、轮询线程、订阅线程与运行中的收发状态不落盘。

### 运行错误提示恢复规则

- `last_error` 表示当前仍需关注的最近一次运行错误，不是永久历史记录；历史详情以模块日志为准。
- 轮询过程中只要一个完整轮询周期内所有实际读取操作及 DataCenter 点值上报均成功，ModbusRTU 会立即清除由轮询产生的 `last_error`。死区过滤不视为失败。
- 轮询成功不会清除写点命令错误；写命令错误仅在后续写命令成功后清除，避免正常轮询掩盖写入故障。
- 链路启动、配置校验、总线打开和删除失败等生命周期错误不会被轮询成功清除，仍需通过对应控制操作处理。
...
- 一轮抄读中只要至少一个点收到合法 RTU 帧并完成解析，就置为 `HEALTHY`；整轮没有任何有效点则置为 `UNHEALTHY`。
- DataCenter 上报失败和死区过滤不影响现场通信健康统计；停止链路不会伪装成运行中的通信异常。
