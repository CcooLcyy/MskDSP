...
void MqttClient::setConfig(const ModbusRTUProto::MqttConfig& config) {
  std::lock_guard<std::mutex> lock(mu_);
  config_ = config;
  hasConfig_ = true;
  channel_.reset();
  stub_.reset();
  LOG_INFO("ModbusRTU MQTT 连接参数已更新: 连接={}:{}(client_id={})",
           config.host(),
           config.port(),
           config.client_id());
}
...
void MqttClient::setStub(std::shared_ptr<MQTTManagerProto::MQTTManagerService::StubInterface> stub) {
  ...
  stub_ = std::move(stub);
  channel_.reset();
  LOG_INFO("ModbusRTU MQTT Stub 已设置");
}
...
grpc::Status MqttClient::RequestAndWait(const std::string& requestTopic,
                                        ...
                                        std::string* error) {
  LOG_INFO("ModbusRTU MQTT 请求响应入口: 请求主题={}, 响应主题={}, 负载长度={}",
           requestTopic,
           responseTopic,
           payload.size());
  if (!hasConfig()) {
  ...
  *req.mutable_connection() = makeConnection();
  ...
  if (!status.ok()) {
    ...
    LOG_ERROR("ModbusRTU MQTT 请求响应 RPC 失败: 连接={}, 请求主题={}, 原因={}",
              connectionLabel(),
              requestTopic,
  ...
  if (!resp.ok()) {
    ...
    LOG_ERROR("ModbusRTU MQTT 请求响应失败: 连接={}, 请求主题={}, 原因={}",
              connectionLabel(),
              requestTopic,
              resp.message());
  ...
  LOG_INFO("ModbusRTU MQTT 请求响应成功: 连接={}, 请求主题={}, 响应主题={}, 负载长度={}",
           connectionLabel(),
           requestTopic,
           responseTopic,
           resp.payload().size());
