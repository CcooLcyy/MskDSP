...
grpc::Status DataCenterClient::DeleteConnection(const std::string &connName) {
  ...
  req.mutable_key()->set_module_name(moduleName_);
  req.mutable_key()->set_conn_name(connName);
...
grpc::Status DataCenterClient::PublishValue(uint32_t connId,
