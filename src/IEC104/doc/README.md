...
- 对时：STATION_ROLE_MASTER 可通过 `time_sync_tag` 订阅触发或 gRPC `SendTimeSync` 主动触发；STATION_ROLE_SLAVE 收到有效对时命令后直接设置系统时间，并返回 IEC104 协议确认
...
- STATION_ROLE_SLAVE 收到有效 `C_CS_NA_1` 后直接设置本机系统时间；设置成功返回 `ACT_CON` 与 `ACT_TERM`，设置失败返回否定 `ACT_CON`。设备容器需具备 `CAP_SYS_TIME`（当前发布脚本使用 `--privileged`）。
...
- 从站收到对时后不向 DataCenter 发布对时事件；`time_sync_tag` 仅用于主站侧的对时发送触发。
...
配置导入与监听校验边界：
- `UpsertLink`、配置导入和持久化恢复只校验链路配置格式、必填字段、端口范围以及点表内容，不要求 `ROLE_SERVER.local.ip` 当前属于本机，也不要求端口当前未被其他进程占用。
- 多条停止状态的服务端链路可以暂时使用相同的监听端点，便于跨设备导入配置后保留点表并逐条修改现场地址；导入和恢复不得因该冲突丢弃链路或点表。
- 当前机器 IP 是否存在、端口是否被占用以及多个服务端链路是否能够同时监听，统一在显式 `StartLink` 或自动启动时检查。检查失败只影响该链路，链路保持停止并记录 `last_error`。
- `0.0.0.0` 仍表示监听所有本机 IPv4 地址，不作为导入时对固定本地 IP 的自动替换。
...
- 只有**变位事件**进入 SOE：单点遥信的状态与品质均未变化时不上送、不落盘、不占用容量；首次（无基线）、状态变位、品质变化、人工模拟值注入与总召快照不受该判定影响，判定不使用点值时标。
...
DataCenter `Publish`（触发主站发送对时，需先配置路由）：
```jsonc
{
  "conn_id": 2001,
  "tag": "clock_sync",
