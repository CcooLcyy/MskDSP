...
- 行为：
  ...
  - 默认交付配置使用 `UPPER` 模式，并在 `auto_start_modules` 中显式列出全部 13 个业务模块，使全新安装、升级或重新安装后即使尚无 SQLite 配置也会启动模块服务
...
- 建议：使用 `CONFIG_PUSHER` 时，自启动列表仅填写 `ConfigPusher`；默认 `UPPER` 交付配置应列出全部业务模块，并继续排除 `ConfigPusher`。
- 注意：这里的“启动模块服务”不等于启动模块内功能；协议链路、AGC/AVC 控制组和 Calc 计算组仍由各模块根据有效持久化配置决定是否恢复运行。
...
- SQLite：`./conf/config.db`
  ...
  - `ModbusTCP/links`、`ModbusTCP/point_tables`
...
```jsonc
{
  ...
  // 显式自动启动全部业务模块；与 UPPER 模式的“SQLite 痕迹自动启动”可同时存在。
  "auto_start_modules": [
    "DataCenter",
    "MQTTManager",
    "IEC104",
