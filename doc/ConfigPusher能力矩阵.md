...
| ModbusTCP 链路与点表 | `modbus_tcp.links[]` | 查询并停止需更新的连接功能，清理 `jsonc` 未声明的旧链路，再调用 `UpsertLink`、`UpsertPointTable` | 链路与点表按目标态覆盖；每条链路独立配置 `host/port/unit_id`，不复用 RTU 传输参数 | 依赖 `ModbusTCP` 与 `DataCenter`；有效点表下发后由 ModbusTCP 自动启动模块内连接功能 | 适合建模为独立的 TCP 主站初始化模板；在线维护仍建议直连 `ModbusTCPService` |
...
- 运行前置编排先查询模块信息与运行态；存在IEC61850目标态时会尽力先启动DataCenter，再启动IEC61850。DataCenter失败不阻止IEC61850，但会阻止仍将其作为硬依赖的其他模块继续下发。
- 进入配置下发阶段后，IEC61850完整目标态在DataCenter路由之前下发；其余配置继续按 `IEC104 -> ModbusRTU -> ModbusTCP -> DLT645 -> AGC -> AVC -> Calc -> DataCenter` 的顺序串行执行。
